...
MotionData IMU::MotionModel(double t)
{
    ...
    // translation
    // twb:  body frame in world frame  // namely world frame to body frame
...
void IMU::testImu(std::string src, std::string dist)
{
    ...
    {
        MotionData imupose_k = imudata[i-1]; //k时刻的数据
        MotionData imupose_k_add1 = imudata[i]; //k+1时刻的数据

        Eigen::Vector3d acc_b_k =imupose_k.imu_acc;
        Eigen::Vector3d gyro_k =imupose_k.imu_gyro; //直接将imu角速度测量值作为imu在body坐标系下的角速度 不考虑噪声
        Eigen::Vector3d acc_b_k_add1 =imupose_k_add1.imu_acc;
        Eigen::Vector3d gyro_k_add1 =imupose_k_add1.imu_gyro; //直接将imu角速度测量值作为imu在body坐标系下的角速度 不考虑噪声
...
//        /// 欧拉积分的acc_w和gyro
//        acc_w = Qwb * acc_b_k + gw; //直接将imu加速度测量值经旋转得到imu在世界坐标系下的加速度 不考虑噪声
//        dtheta_half =  gyro_k * dt /2.0;
//        dq.w() = 1;
//        dq.x() = dtheta_half.x();
//        dq.y() = dtheta_half.y();
//        dq.z() = dtheta_half.z();
//        dq.normalize();
        ...
        /// 中值积分的acc_w和gyro
        acc_w = Qwb * ((acc_b_k+acc_b_k_add1)/2.0) + gw; //直接将imu加速度测量值经旋转得到imu在世界坐标系下的加速度 不考虑噪声
        dtheta_half = ((gyro_k+gyro_k_add1)/2) * dt /2.0;
        dq.w() = 1;
        dq.x() = dtheta_half.x();
        dq.y() = dtheta_half.y();
        dq.z() = dtheta_half.z();
        ...
        Pwb = Pwb + Vw * dt + 0.5 * dt * dt * acc_w; //得到k时刻位置
        Vw = Vw + acc_w * dt; //得到k时刻速度
        Qwb = Qwb * dq; //得到k时刻旋转

        //　按着imu postion, imu quaternion , cam postion, cam quaternion 的格式存储，由于没有cam，所以imu存了两次
        save_points<<imupose_k_add1.timestamp<<" "
                   <<Qwb.w()<<" "
                   <<Qwb.x()<<" "
                   <<Qwb.y()<<" "
                   <<Qwb.z()<<" "
