...
public:
    ...
    //！！！
    //前一个维度是状态变量维度(状态变量在g2o中有时会进一步封装以升级，如将6维李代数kesi封装为李群) 后一个维度是参与优化时的具体维度(最小维度)
    //local_dimension：局部维度 指的是参与优化时的最小的参数化的维度，比如对于旋转，用旋转矩阵则传进来的是12维的变量，但其实际可变化的自由度只有3,所以local指的是可自由变化的最小维度
    //所以平移+四元数有7个变量，只需要告诉它6自由度的变量
