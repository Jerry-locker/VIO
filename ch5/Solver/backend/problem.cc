...
bool Problem::AddVertex(std::shared_ptr<Vertex> vertex)
{
    if (verticies_.find(vertex->Id()) != verticies_.end()) {
        ...
        //一旦add了Vertex会自动记录该顶点的id 这样便知道该顶点对应的雅克比在H矩阵中的哪个位置
        //比如第一个顶点假如是一个6自由度的位姿 则其对应的起始坐标在H矩阵中就是(0,0) 第二个位姿顶点对应的雅克比就要从(6,6)开始
        //因此每当一个顶点放进来 都要关心是否加入成功 以及放在了哪个位置
    }
...
bool Problem::Solve(int iterations)
{
    ...
    TicToc t_solve;
    // 1.得出每个状态变量对应雅克比在H和b中的索引并计算H总维度
    SetOrdering(); //统计优化变量总的local_dimension即可知H总维度
    /// 2.遍历edge并构建H矩阵和b矩阵
    MakeHessian(); //每迭代一次都要重新make
    // 3.计算LM阻尼因子初值
    ComputeLambdaInitLM();
    // 4.迭代优化求解
    bool stop = false;
...
//            AddLambdatoHessianLM();  //setLambda
            ...
            /// 5.求解线性方程
            SolveLinearSystem(); //H+lamda*I的操作放进该函数中去了
...
{
    // 每次重新计数
    ordering_poses_ = 0;      //pose的local_dimension的总维数
    ordering_landmarks_ = 0;  //landmark的local_dimension的总维数
    ordering_generic_ = 0;    //pose+landmark的local_dimension的总维数
    int debug = 0;
...
void Problem::MakeHessian()
{
    TicToc t_h;
    // 1.构建H空矩阵和b空矩阵
    ulong size = ordering_generic_;   //local_dimension的总维数
    ...
    for (auto &edge: edges_)
    {
        ...
        for (size_t i = 0; i < verticies.size(); ++i)
        {
            ...
            if (v_i->IsFixed()) continue;        //Hessian里不需要添加它的信息，也就是它的雅克比为0

            auto jacobian_i = jacobians[i];
            ulong index_i = v_i->OrderingId();   //OrderingId就是0、6、12、...、51、52、53、54(这边是逆深度1维)等等这种 即每个优化变量在H矩阵中的索引
            ulong dim_i = v_i->LocalDimension();

            MatXX JtW = jacobian_i.transpose() * edge.second->Information();  //权重矩阵均设为单位矩阵
            for (size_t j = i; j < verticies.size(); ++j)
            {
                ...
                // 所有的信息矩阵叠加起来
                // TODO:: home work1. 完成 H index 的填写.
                H.block(index_i, index_j, dim_i, dim_j).noalias() += hessian;  //noalias()函数用于解决矩阵运算左右均带变量时的混淆

                if (j != i)
                {
                // 对称的下三角
                // TODO:: home work1. 完成 H index 的填写.  00 01 02 11 12 22
    ...
    //前面每个子H和子b由填充操作得到，最终的总H和总b要对子H和子b进行累加才能得到
    Hessian_ = H;
    b_ = b;
...
void Problem::SolveLinearSystem()
{
    if (problemType_ == ProblemType::GENERIC_PROBLEM)
    {
        ...
        {
            H(i, i) += currentLambda_;  //H矩阵加阻尼因子
        }
//        delta_x_ = PCGSolver(H, b_, H.rows() * 2);
        delta_x_ = Hessian_.inverse() * b_;  //非slam问题不是稀疏的因此直接求逆 直接求逆就可得增量
    ...
    {
        // SLAM问题采用舒尔补的计算方式
        // step1: schur marginalization --> Hpp_shur, bpp_shur

        //marg路标点留下数量少的相机位姿
        int reserve_size = ordering_poses_;   //最终的起始索引正好为当前总的local_dimension维数
        int marg_size = ordering_landmarks_;

        // TODO:: home work2. 完成矩阵块取值，Hll，Hpl，Hlp，bpp，bll
        MatXX Hll = Hessian_.block(reserve_size, reserve_size, marg_size, marg_size);  //路标和路标
        MatXX Hpl = Hessian_.block(0, reserve_size, reserve_size, marg_size);          //位姿和路标
        MatXX Hlp = Hessian_.block(reserve_size, 0, marg_size, reserve_size);
        VecX bpp = b_.segment(0, reserve_size);          //位姿增量对应的b项
        VecX bll = b_.segment(reserve_size, marg_size);  //路标增量对应的b项

        //Hll是对角线矩阵，它的求逆可以直接为对角线块分别求逆，如果是逆深度，对角线块为1维的，则直接为对角线的倒数，这里可以加速！
        MatXX Hll_inv(MatXX::Zero(marg_size, marg_size));
        for (auto landmarkVertex : idx_landmark_vertices_)
        {
        ...
        // TODO:: home work2. 完成舒尔补Hpp_shur, bp_shur代码
        MatXX tempH = Hpl * Hll_inv;
        H_pp_schur_ = Hessian_.block(0, 0, reserve_size, reserve_size) - tempH * Hlp; //取哪一块？
        b_pp_schur_ = bpp - tempH * bll;

        // step2: solve Hpp_shur * delta_x = bp_shur
        VecX delta_x_pp(VecX::Zero(reserve_size));
        // PCG Solver
        for (ulong i = 0; i < ordering_poses_; ++i)
        {
            H_pp_schur_(i, i) += currentLambda_;   //其实我认为更严谨的应该是先整个H加上lamdaI再利用舒尔补加速求解
        }

        int n = H_pp_schur_.rows() * 2;                       // 迭代次数
        delta_x_pp = PCGSolver(H_pp_schur_, b_pp_schur_, n);  // 哈哈，小规模问题，搞 pcg 花里胡哨(其实直接用H的逆去求即可)
        delta_x_.head(reserve_size) = delta_x_pp;
//        std::cout << delta_x_pp.transpose() << std::endl;

        // TODO:: home work. step3: solve landmark 求landmark对应的增量是多少
        VecX delta_x_ll(marg_size);
        delta_x_ll = Hll_inv * (bll - Hlp * delta_x_pp);
...
{
    // Add marg test
    int idx = 1;            // marg 中间那个变量 id为0、1、2,则要marg掉的中间的那个变量的id为1
    int dim = 1;            // marg 变量的维度
    int reserve_size = 3;   // 总共变量的维度
    double delta1 = 0.1 * 0.1; //变量 房间1 2 3温度的方差 均服从高斯分布
